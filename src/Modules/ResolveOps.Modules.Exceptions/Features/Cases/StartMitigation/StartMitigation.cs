...
public sealed record StartMitigationRequest(string? Plan, string ConcurrencyStamp, string? DetailsJson = null);
public sealed record StartMitigationCommand(Guid CaseId, string? Plan, string ConcurrencyStamp, string? DetailsJson = null);
...
public sealed class StartMitigationEndpoint : IEndpoint
{
    public void MapEndpoint(IEndpointRouteBuilder app)
    {
        app.MapPost("/api/exception-cases/{id:guid}/start-mitigation", async (
            Guid id,
            StartMitigationRequest request,
            IValidator<StartMitigationCommand> validator,
            StartMitigationHandler handler,
            HttpContext httpContext,
            CancellationToken cancellationToken) =>
        {
            var command = new StartMitigationCommand(id, request.Plan, request.ConcurrencyStamp, request.DetailsJson);
