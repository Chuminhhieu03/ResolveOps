...
internal sealed class ResolveQuarantinedEventHandler
{
    ...
    public ResolveQuarantinedEventHandler(
        ...
        CancellationToken cancellationToken)
    {
        ...
        if (quarantinedEvent.Status != QuarantinedEventStatus.Quarantined)
        {
            return DomainError.Failure(
                ...
                $"Quarantined event cannot be resolved because its current status is '{quarantinedEvent.Status}'.");
        }

        var shipment = await _dbContext.Shipments
            .FirstOrDefaultAsync(s => s.Id == request.ShipmentId && s.TenantId == tenantId, cancellationToken);
        ...
        var eventType = parsed?.EventType ?? TrackingEventType.InTransit;
        var eventCode = parsed?.EventCode;
        var occurredAt = parsed?.OccurredAtUtc ?? receipt.ReceivedAtUtc;
        ...
        quarantinedEvent.Resolve(shipment.Id, userId, _timeProvider);
